...
void Elfin10lPose1DynamicsModel::gravityTerm( double* g_out, const double* parms, const double* q )
{
  double x0 = cos(q[1]);
  double x1 = cos(q[3]);
  double x2 = sin(q[1]);
  ...
  double x32 = x16*x31;
  double x33 = parms[59]*x15 + parms[60]*x25 + x16*x24 + x22*x28 + x29*x30 - 0.2*x32;
  ...
  double x40 = x16*x29;
  double x41 = parms[58]*x27 + parms[60]*x20 + x16*x28 + x21*x24 + x30*x31 + 0.2*x40;
  ...
  double x48 = x17*x47;
  double x49 = parms[33]*x12 + x1*x39 + x1*x44 + x18*x42 - 0.52*x48;
  double x50 = x1*x47;
  double x51 = parms[32]*x13 + parms[34]*x8 + x1*x42 + x17*x39 + x17*x44 + 0.52*x50;
//
  g_out[0] = x0*(-parms[21]*x3 + x38*x4 + x49*x7) + x2*(-0.48*((x0)*(x0)) - 0.48*((x2)*(x2)))*(x1*x43 + x48) + x2*(parms[21]*x5 + x38*x6 + x4*x49);
  g_out[1] = -parms[19]*x3 + parms[20]*x5 - 0.48*parms[22]*x3 - 0.48*x4*(parms[35]*x8 + x18*x43 + x50) - x51 - 0.48*x6*(parms[35]*x12 - parms[48]*x13 - x10*x45 - x36*x46);
  g_out[2] = x51;
  g_out[3] = x37;
  g_out[4] = x41;
  g_out[5] = x34;
