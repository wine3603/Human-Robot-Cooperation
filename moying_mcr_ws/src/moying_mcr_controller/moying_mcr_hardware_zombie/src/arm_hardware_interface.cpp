...
#include "moying_mcr_hardware_zombie/arm_hardware_interface.hpp"
  ...
  const hardware_interface::HardwareInfo & info)
{
  ...
  if(arm_config["if_name"]){
    ...
    ecat_manager_ = std::make_shared<elfin_ethercat_driver_zombie::EtherCatManager>(arm_config["if_name"].as<std::string>());
  }
  else{
  ...
  arm_handle_ = std::make_shared<moying_mcr_hardware_zombie::ArmEthercatDriver>(ecat_manager_,arm_ethercat_node,arm_config);
...
PLUGINLIB_EXPORT_CLASS(
  moying_mcr_hardware_zombie::ArmHardwareInterface, hardware_interface::SystemInterface)
