...
controller_interface::return_type MecanumbotDriveController::update(const rclcpp::Time & time, const rclcpp::Duration & period)
{
    ...
    Twist twist;
    twist.linear.x = direction_*1.0*(*velocity_command)->linear.x;
    twist.linear.y = -1.0*(*velocity_command)->linear.y;
    twist.angular.z = -1.0*(*velocity_command)->angular.z;

    // RCLCPP_INFO_STREAM(get_node()->get_logger(),"twist.linear.x "<<twist.linear.x ;
    // RCLCPP_INFO_STREAM(get_node()->get_logger(),"twist.linear.x "<<twist.linear.x ;

    if(twist.linear.x >= max_linear_)
        twist.linear.x =  max_linear_;
    if(twist.linear.y >= max_linear_)
        twist.linear.y =  max_linear_;
    if(twist.angular.z >= max_radial_)
        twist.angular.z =  max_radial_;
    ...
    if(twist.angular.z <= -max_radial_)
        twist.angular.z =  -max_radial_;
    twist.linear.x = twist.linear.x;//*linear_scale_;
    twist.linear.y = twist.linear.y;//*linear_scale_;
    twist.angular.z = twist.angular.z;//*radial_scale_;
    ...
    // RCLCPP_INFO_STREAM(get_node()->get_logger(),"fl:"<<fl_wheel_velocity<<" fr:"<<fr_wheel_velocity);
    // RCLCPP_INFO_STREAM(get_node()->get_logger(),"rl:"<<rl_wheel_velocity<<" rr:"<<rr_wheel_velocity);
    // RCLCPP_INFO_STREAM(get_node()->get_logger(),"==================");
    // fl_wheel_->set_velocity(40.0);
    // fr_wheel_->set_velocity(4.0);
    // rl_wheel_->set_velocity(4.0);
    // rr_wheel_->set_velocity(4.0);
...
controller_interface::CallbackReturn MecanumbotDriveController::on_configure(const rclcpp_lifecycle::State &)
{
    ...
    linear_scale_ = get_node()->get_parameter("linear_scale").as_double();
    radial_scale_ = get_node()->get_parameter("radial_scale").as_double();
    max_linear_ = get_node()->get_parameter("max_linear").as_double();
    max_radial_ = get_node()->get_parameter("max_radial").as_double();
    direction_ = get_node()->get_parameter("direction").as_double();


    if(linear_scale_> 10.0 || linear_scale_< 0.0){
        RCLCPP_ERROR(get_node()->get_logger(), "'linear scale ' parameter was not fit");
        return controller_interface::CallbackReturn::ERROR;
    }
    if(radial_scale_> 10.0 || radial_scale_< 0.0){
        RCLCPP_ERROR(get_node()->get_logger(), "'radial scale ' parameter was not fit");
        return controller_interface::CallbackReturn::ERROR;
    }
...
{
    fl_wheel_->set_velocity(0.0);
    fr_wheel_->set_velocity(0.0);
    rl_wheel_->set_velocity(0.0);
    rr_wheel_->set_velocity(0.0);
    return controller_interface::CallbackReturn::SUCCESS;
}
...
std::shared_ptr<MecanumbotWheel> MecanumbotDriveController::get_wheel(const std::string & wheel_joint_name)
{
    ...
    RCLCPP_INFO(get_node()->get_logger(),"wheel_joint_name:%s",wheel_joint_name.c_str());
    for(const auto& inf : state_interfaces_)
    {
        RCLCPP_INFO(get_node()->get_logger(), "%s,%s",inf.get_name().c_str(), inf.get_prefix_name().c_str());
    }
