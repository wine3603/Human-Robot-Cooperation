...
#include "mmco_dual_controller/mmco_dual_controller.hpp"
...
using ControllerReferenceMsg = mmco_dual_controller::MMCODualController::ControllerReferenceMsg;
...
namespace mmco_dual_controller
{
MMCODualController::MMCODualController() : controller_interface::ControllerInterface() {}

controller_interface::CallbackReturn MMCODualController::on_init()
{
  control_mode_.initRT(control_mode_type::FAST);

  try
  {
    param_listener_ = std::make_shared<mmco_dual_controller::ParamListener>(get_node());
  }
  ...
  cmd_publisher_ = get_node()->create_publisher<geometry_msgs::msg::Twist>("/mcr/mecanum_controller/cmd_vel", 10);

  mmInterface_ = std::make_shared<mmco_dual_interface::MMCODualInterface>(taskFile, libFold, urdfFile);
  ...
  mcr_pose_sub_ = get_node()->create_subscription<geometry_msgs::msg::PoseStamped>(
    "/vrpn/mcr/pose",  // 话题名称
    10,  // QoS 队列深度
    std::bind(&MMCODualController::mcrposeCallback, this, std::placeholders::_1)
  );
...
// void MMCODualController::tf_callback() {
//         std::lock_guard<std::mutex> lock(transform_mutex_);
//         try {
//             auto transform_stamped = tf_buffer_->lookupTransform("world", "mcr/base_link", tf2::TimePointZero);
...
//             RCLCPP_WARN_ONCE(get_node()->get_logger(), "Could not transform 'world' to '/mcr/base_link': %s", ex.what());
//         }
// }
void MMCODualController::mcrposeCallback(const geometry_msgs::msg::PoseStamped::SharedPtr msg) 
{
...
void MMCODualController::setupMpc()
{
...
{  
  ...
  ref_subscriber_ = get_node()->create_subscription<ControllerReferenceMsg>(
    ...
    std::bind(&MMCODualController::reference_callback, this, std::placeholders::_1));
...
void MMCODualController::reference_callback(const std::shared_ptr<ControllerReferenceMsg> msg)
{
...
controller_interface::InterfaceConfiguration MMCODualController::command_interface_configuration() const
{
  controller_interface::InterfaceConfiguration command_interfaces_config;
  command_interfaces_config.type = controller_interface::interface_configuration_type::INDIVIDUAL;

  command_interfaces_config.names.reserve(params_.joints.size());
  std::string left_arm = "left_arm_";
  std::string right_arm = "right_arm_";
  for (const auto & joint : params_.joints)
  {
    if(params_.use_chain_interface == false){
      command_interfaces_config.names.push_back(joint + "/" + "velocity");
      // RCLCPP_INFO(get_node()->get_logger(),"config [Command Interface]:%s/velocity",joint.c_str());
    }
    else{
      if(joint.find("left")!= std::string::npos){
        command_interfaces_config.names.push_back(left_arm+params_.chain_velocity_interface_name+"/"+joint + "/" + "velocity");
        // RCLCPP_INFO(get_node()->get_logger(),"config [Command Interface]:%s%s/%s/velocity",left_arm.c_str(),params_.chain_velocity_interface_name.c_str() ,joint.c_str());
      }
      else if(joint.find("right")!= std::string::npos){
        command_interfaces_config.names.push_back(right_arm+params_.chain_velocity_interface_name+"/"+joint + "/" + "velocity");
        // RCLCPP_INFO(get_node()->get_logger(),"config [Command Interface]:%s%s/%s/velocity",right_arm.c_str(),params_.chain_velocity_interface_name.c_str() ,joint.c_str());
      }
    }
  }
...
controller_interface::InterfaceConfiguration MMCODualController::state_interface_configuration() const
{
...
ocs2::TargetTrajectories MMCODualController::goalPoseToTargetTrajectories(const std::vector<Eigen::Vector3d>& position, const std::vector<Eigen::Quaterniond>& orientation,
...
controller_interface::CallbackReturn MMCODualController::on_activate(
  const rclcpp_lifecycle::State & /*previous_state*/)
{
  ...
  // if(use_gui_target)
  // {
  //   auto bound_function = std::bind(&MMCODualController::goalPoseToTargetTrajectories, this, std::placeholders::_1, std::placeholders::_2,std::placeholders::_3);
  //   targetPoseCommandInterface_ = std::make_shared<mmco_dual_interface::MMCODualTargetTrajectoriesInteractiveMarker>(target_node_,robot_name_,bound_function);
  ...
  auto bound_function = std::bind(&MMCODualController::goalPoseToTargetTrajectories, this, std::placeholders::_1, std::placeholders::_2,std::placeholders::_3);
  targetJoyPoseCommandInterface_= std::make_shared<mmco_dual_interface::MMCODualJoyTargetTrajectories>(target_node_,robot_name_,bound_function,task_file_);
  targetThread_ = std::thread([&](){
  ...
  // std::cout<<"command_interface的大小:"<<command_interfaces_.size()<<std::endl;
    ...
    command_interfaces_[7].set_value(0);
    command_interfaces_[8].set_value(0);
    command_interfaces_[9].set_value(0);
    command_interfaces_[10].set_value(0);
    command_interfaces_[11].set_value(0);
...
controller_interface::CallbackReturn MMCODualController::on_deactivate(
  const rclcpp_lifecycle::State & /*previous_state*/)
{
...
void MMCODualController::updateStateEstimation(const rclcpp::Time time,const rclcpp::Duration &period)
{
  double roll,pitch,yaw;
  double posx,posy;
  ...
  ocs2::vector_t state(mmInterface_->getMMCODualModuelInfo().stateDim);
  state(0) = posx;
  state(1) = posy;
  ocs2::scalar_t yawLast = currentObservation_.state(2);
  state(2) = yawLast + angles::shortest_angular_distance(yawLast, yaw) ;

  state(3) = state_interfaces_[0].get_value();
  state(4) = state_interfaces_[1].get_value();
  state(5) = state_interfaces_[2].get_value();
  state(6) = state_interfaces_[3].get_value();
  state(7) = state_interfaces_[4].get_value();
  state(8) = state_interfaces_[5].get_value();
  state(9) = state_interfaces_[6].get_value();
  state(10) = state_interfaces_[7].get_value();
  state(11) = state_interfaces_[8].get_value();
  state(12) = state_interfaces_[9].get_value();
  state(13) = state_interfaces_[10].get_value();
  state(14) = state_interfaces_[11].get_value();
  currentObservation_.state = state;
}

controller_interface::return_type MMCODualController::update(
  const rclcpp::Time & time, const rclcpp::Duration & period)
{
  ...
  if(first_use == false)
  {
    ...
    const ocs2::vector_t zeroInput =
        ocs2::vector_t::Zero(mmInterface_->getMMCODualModuelInfo().inputDim);
  ...
  command_interfaces_[0].set_value(optimizedInput(3));
  command_interfaces_[1].set_value(optimizedInput(4));
  command_interfaces_[2].set_value(optimizedInput(5));
  command_interfaces_[3].set_value(optimizedInput(6));
  command_interfaces_[4].set_value(optimizedInput(7));
  command_interfaces_[5].set_value(optimizedInput(8));
  command_interfaces_[6].set_value(optimizedInput(9));
  command_interfaces_[7].set_value(optimizedInput(10));
  command_interfaces_[8].set_value(optimizedInput(11));
  command_interfaces_[9].set_value(optimizedInput(12));
  command_interfaces_[10].set_value(optimizedInput(13));
  command_interfaces_[11].set_value(optimizedInput(14));
...
PLUGINLIB_EXPORT_CLASS(
  mmco_dual_controller::MMCODualController, controller_interface::ControllerInterface)
