#include "mmco_dual_interface/command/joy_target_trajectories.h"
...
/******************************************************************************************************/
/******************************************************************************************************/
MMCODualJoyTargetTrajectories::MMCODualJoyTargetTrajectories(
  ...
  timer_ = node_->create_wall_timer(
      100ms, std::bind(&MMCODualJoyTargetTrajectories::timer_callback,this));
  ...
  std::string joy_topic_name; // 1. 先声明变量，不要直接赋值

  // 2. 关键修改：这里的 Key 必须是 "topic_name"，因为上一级就是用这个名字传进来的！
  if(node_->has_parameter("topic_name")) {
      joy_topic_name = node_->get_parameter("topic_name").as_string();
      // 可以加一行日志，确认读到了什么
      RCLCPP_INFO(node_->get_logger(), "Joy topic set to: %s", joy_topic_name.c_str());
  } else {
      joy_topic_name = "/mcr/joy"; // 默认值
      RCLCPP_WARN(node_->get_logger(), "No 'topic_name' param found, using default: /mcr/joy");
  }

  // 3. 创建订阅
  joy_command_subsciption_ = node_->create_subscription<sensor_msgs::msg::Joy>(joy_topic_name, rclcpp::SystemDefaultsQoS(), [this](const sensor_msgs::msg::Joy::SharedPtr twist) 
  {
    joy_command_ptr_.writeFromNonRT(twist);
  });
  ...
  RCLCPP_INFO_STREAM(node_->get_logger(),"[MMCODualJoyTargetTrajectory] Loading...");
  boost::filesystem::path taskFilePath(taskFile);
  if (boost::filesystem::exists(taskFilePath)) {
    RCLCPP_INFO_STREAM(node_->get_logger(),"[MMCODualJoyTargetTrajectory] Loading task from:"<<taskFilePath);
  } else {
    RCLCPP_ERROR_STREAM(node_->get_logger(),"[MMCODualJoyTargetTrajectory] Can't loading task file");
...
/******************************************************************************************************/
/******************************************************************************************************/
void MMCODualJoyTargetTrajectories::timer_callback() 
{
