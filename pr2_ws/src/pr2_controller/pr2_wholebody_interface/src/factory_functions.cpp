...
PR2WBCModuleInfo createPR2WBCModuleInfo(const ocs2::PinocchioInterface& interface, const std::string& baseFrame,
                                            const std::string& eleFrame, const std::string& ereFrame)
{
  const auto& model = interface.getModel();
  PR2WBCModuleInfo info;
  info.stateDim = model.nq;
  std::cerr << "\n #### nq:" << info.stateDim << std::endl;
  info.inputDim = info.stateDim;
  std::cerr << "\n #### inputDim:" << info.inputDim << std::endl;
  info.torsoLiftDim = 1;
  std::cerr << "\n #### torsoLiftDim:" << info.torsoLiftDim << std::endl;
  info.leftArmDim = (info.stateDim - info.torsoLiftDim - 3) / 2;
  std::cerr << "\n #### leftArmDim:" << info.leftArmDim << std::endl;
  info.rightArmDim = (info.stateDim - info.torsoLiftDim - 3) / 2;
  std::cerr << "\n #### rightArmDim:" << info.rightArmDim << std::endl;
  info.baseDim = info.inputDim - info.leftArmDim - info.rightArmDim - info.torsoLiftDim;
  std::cerr << "\n #### baseDim:" << info.baseDim << std::endl;
