...
#include "pr2_wholebody_interface/constraint/base_constraint.h"
#include "pr2_wholebody_interface/constraint/self_collision_constraint.h"
...
#include "pr2_wholebody_interface/cost/quadratic_input_cost.h"
// Boost
...
PR2WBCInterface::PR2WBCInterface(const std::string& taskFile, const std::string& libraryFolder,
                                      const std::string& urdfFile)
{
  boost::filesystem::path taskFilePath(taskFile);
  if (boost::filesystem::exists(taskFilePath)) {
    std::cerr << "[PR2WBCInterface] Loading task file: " << taskFilePath << std::endl;
  } else {
    throw std::invalid_argument("[PR2WBCInterface] Task file not found: " + taskFilePath.string());
  }
  boost::filesystem::path urdfFilePath(urdfFile);
  if (boost::filesystem::exists(urdfFilePath)) {
    std::cerr << "[PR2WBCInterface] Loading Pinocchio model from: " << urdfFilePath << std::endl;
  } else {
    throw std::invalid_argument("[PR2WBCInterface] URDF file not found: " + urdfFilePath.string());
  }
  boost::filesystem::path libraryFolderPath(libraryFolder);
  boost::filesystem::create_directories(libraryFolderPath);
  std::cerr << "[PR2WBCInterface] Generated library path: " << libraryFolderPath << std::endl;
  ...
  pr2WBCModuleInfo_ = pr2_wholebody_interface::createPR2WBCModuleInfo(*pinocchioInterfacePtr_,baseFrame, eleFrame,ereFrame);
  ...
  initialState_.setZero(pr2WBCModuleInfo_.stateDim);
  const int baseStateDim = pr2WBCModuleInfo_.baseDim;
  const int torsoLiftStateDim = pr2WBCModuleInfo_.torsoLiftDim;
  const int rightArmStateDim = pr2WBCModuleInfo_.rightArmDim;
  const int leftArmStateDim = pr2WBCModuleInfo_.leftArmDim;
  if (baseStateDim > 0) {
  ...
  ocs2::vector_t initialTorsoLiftState = ocs2::vector_t::Zero(torsoLiftStateDim);
  ocs2::loadData::loadEigenMatrix(taskFile, "initialState.torsoLift", initialTorsoLiftState);
  initialState_.segment(baseStateDim, torsoLiftStateDim) = initialTorsoLiftState;
  ...
  initialState_.segment(baseStateDim + torsoLiftStateDim, rightArmStateDim) = initialRightArmState;
  ...
  initialState_.segment(baseStateDim + torsoLiftStateDim + rightArmStateDim, leftArmStateDim) = initialLeftArmState;
  ...
  problem_.stateSoftConstraintPtr->add("endLeftEffector", getEndEffectorConstraint(*pinocchioNoBaseInterfacePtr_, taskFile, "endLeftEffector",
                                                                                usePreComputation, libraryFolder, recompileLibraries,pr2WBCModuleInfo_.eleFrame));
  ...
  problem_.stateSoftConstraintPtr->add("endRightEffector", getEndEffectorConstraint(*pinocchioNoBaseInterfacePtr_, taskFile, "endRightEffector",
                                                                                usePreComputation, libraryFolder, recompileLibraries,pr2WBCModuleInfo_.ereFrame));
  ...
  problem_.dynamicsPtr.reset(
          new PR2WBCDynamics(pr2WBCModuleInfo_, "dynamics", libraryFolder, recompileLibraries, true));

  if (usePreComputation) {
    problem_.preComputationPtr.reset(new PR2WBCPreComputation(*pinocchioInterfacePtr_, pr2WBCModuleInfo_));
  }
  ...
  // Initialization
  initializerPtr_.reset(new DefaultInitializer(pr2WBCModuleInfo_.inputDim));
}

/******************************************************************************************************/
/******************************************************************************************************/
/******************************************************************************************************/
std::unique_ptr<ocs2::StateInputCost> PR2WBCInterface::getQuadraticInputCost(const std::string& taskFile)
{
  ocs2::matrix_t R = ocs2::matrix_t::Zero(pr2WBCModuleInfo_.inputDim, pr2WBCModuleInfo_.inputDim);
  const int baseInputDim = pr2WBCModuleInfo_.baseDim ;
  const int leftArmStateDim = pr2WBCModuleInfo_.leftArmDim;
  const int rightArmStateDim = pr2WBCModuleInfo_.rightArmDim;
  const int torsoLiftStateDim = pr2WBCModuleInfo_.torsoLiftDim;
  ...
  ocs2::matrix_t R_torsoLift = ocs2::matrix_t::Zero(torsoLiftStateDim, torsoLiftStateDim);
  ocs2::loadData::loadEigenMatrix(taskFile, "inputCost.R.torsoLift", R_torsoLift);
  // right arm joints 
  ...
  // add in R
  const int armDim = leftArmStateDim + rightArmStateDim + torsoLiftStateDim;
  ocs2::matrix_t R_temp = ocs2::matrix_t::Zero(armDim,armDim);
  R_temp.block(0, 0, torsoLiftStateDim, torsoLiftStateDim) = R_torsoLift;
  R_temp.block(torsoLiftStateDim, torsoLiftStateDim, rightArmStateDim, rightArmStateDim) = R_rightArm;
  R_temp.block(torsoLiftStateDim + rightArmStateDim,
              torsoLiftStateDim + rightArmStateDim,
              leftArmStateDim,
              leftArmStateDim) = R_leftArm;
  ...
  return std::make_unique<pr2_wholebody_interface::QuadraticInputCost>(std::move(R), pr2WBCModuleInfo_.stateDim);
}

/******************************************************************************************************/
/******************************************************************************************************/
/******************************************************************************************************/
std::unique_ptr<ocs2::StateInputCost> PR2WBCInterface::getJointLimitSoftConstraint(const PinocchioInterface& pinocchioInterface,
                                                                                        const std::string& taskFile)
{
  ...
  const int leftArmStateDim = pr2WBCModuleInfo_.leftArmDim;
  const int rightArmStateDim = pr2WBCModuleInfo_.rightArmDim;
  const int armStateDim = leftArmStateDim + rightArmStateDim + pr2WBCModuleInfo_.torsoLiftDim;
  const int baseInputDim = pr2WBCModuleInfo_.baseDim ;
  const int leftArmInputDim = pr2WBCModuleInfo_.leftArmDim;
  const int rightArmInputDim = pr2WBCModuleInfo_.rightArmDim ;
  const int armInputDim = leftArmInputDim + rightArmInputDim + pr2WBCModuleInfo_.torsoLiftDim;
  ...
  {
    vector_t lowerBound = vector_t::Zero(pr2WBCModuleInfo_.inputDim);
    vector_t upperBound = vector_t::Zero(pr2WBCModuleInfo_.inputDim);
    ...
    inputLimits.reserve(pr2WBCModuleInfo_.inputDim);
    std::cerr << " #### =============================FOR======================================\n";
    for (int i = 0; i < pr2WBCModuleInfo_.inputDim; ++i) {
  ...
  boxConstraints->initializeOffset(0.0, vector_t::Zero(pr2WBCModuleInfo_.stateDim), vector_t::Zero(pr2WBCModuleInfo_.inputDim));
  return boxConstraints;
}

std::unique_ptr<ocs2::StateCost> PR2WBCInterface::getEndEffectorConstraint(const PinocchioInterface& pinocchioInterface,
                                                                                ...
                                                                                bool recompileLibraries,const std::string endEffectorName)
{
  ...
  if (usePreComputation) {
    std::cerr << " #### ==========================PR2WBCPinocchioMapping=======================\n";
    PR2WBCPinocchioMapping pinocchioMapping(pr2WBCModuleInfo_);
    ...
    #ifdef mode
    PR2WBCPinocchioMappingCppAd pinocchioMappingCppAd(pr2WBCModuleInfo_);
    PinocchioEndEffectorKinematicsCppAd eeKinematics(pinocchioInterface, pinocchioMappingCppAd, {endEffectorName},
                                                     pr2WBCModuleInfo_.stateDim-3, pr2WBCModuleInfo_.inputDim-3,
                                                     prefix+"_end_effector_kinematics", libraryFolder, recompileLibraries, false);
    #else
    PR2WBCWBPinocchioMappingCppAd pinocchioMappingCppAd(pr2WBCModuleInfo_);
    std::cerr <<"状态维度："<< pr2WBCModuleInfo_.stateDim<<"line353"<<"输入维度："<<pr2WBCModuleInfo_.inputDim<<"========\n";
    PinocchioEndEffectorKinematicsCppAd eeKinematics(pinocchioInterface, pinocchioMappingCppAd, {endEffectorName},
                                                     pr2WBCModuleInfo_.stateDim, pr2WBCModuleInfo_.inputDim,
                                                     prefix+"_end_effector_kinematics", libraryFolder, recompileLibraries, false);
...
std::unique_ptr<ocs2::StateCost> PR2WBCInterface::getBaseConstraint(const PinocchioInterface& pinocchioInterface,
                                                                                const std::string& taskFile, const std::string& prefix,
                                                                                bool usePreComputation, const std::string& libraryFolder,
                                                                                bool recompileLibraries)
{
  ...
  std::unique_ptr<StateConstraint> constraint;
  if (usePreComputation) {
    PR2WBCWBPinocchioMapping pinocchioMapping(pr2WBCModuleInfo_);
...
/******************************************************************************************************/
/******************************************************************************************************/
std::unique_ptr<ocs2::StateCost> PR2WBCInterface::getSelfCollisionConstraint(const PinocchioInterface& pinocchioInterface,
  ...
  std::unique_ptr<StateConstraint> constraint;
  if (usePreComputation) {
    constraint = std::make_unique<PR2WBCSelfCollisionConstraint>(PR2WBCPinocchioMapping(pr2WBCModuleInfo_),
                                                                            std::move(geometryInterface), minimumDistance);
  } else {
    constraint = std::make_unique<SelfCollisionConstraintCppAd>(
        pinocchioInterface, PR2WBCPinocchioMapping(pr2WBCModuleInfo_), std::move(geometryInterface), minimumDistance,
