...
```
```bash
#if your mujoco install on the path“opt/mujoco”
#you can follow ：
cd unitree_mujoco/simulate/
ln -s /opt/mujoco mujoco
#make sure mujoco includes files(bin\inlcude\lib\share\simulate)
[reference](https://github.com/White-Zhao/mujoco_reference.git)


# if you do not downloawd Mujoco，here Mujoco Download tips：
# 1. 进入源码目录
cd ~/mujoco   # 或者您的源码文件夹名，如 mujoco-3.3.6

# 2. 创建并进入构建目录（标准操作，保持源码干净）
mkdir build
cd build
...
# 这里我们同时指定：
# (a) ".." 代表源码在上级目录
# (b) "-DCMAKE_INSTALL_PREFIX=/opt/mujoco" 指定未来安装到哪里
# (c) "-DCMAKE_BUILD_TYPE=Release" 建议加上，编译发行版，运行速度最快
cmake .. -DCMAKE_INSTALL_PREFIX=/opt/mujoco -DCMAKE_BUILD_TYPE=Release

# 4. 开始编译 (Build)
# "-j$(nproc)" 会自动使用你 CPU 的最大核心数进行多线程编译，速度最快
cmake --build . -j$(nproc)
#for example
cmake --build . -j1

# 5. 正式安装 (Install)
# 因为 /opt 需要管理员权限，所以这里必须加 sudo
sudo cmake --install 
```
...
```yaml
# Name of the robot to be loaded by the simulator
# Options: "pr2", "moying" (located in the pr2 \ moying  directory)
robot: "pr2"

# Robot simulation scene file
# Refers to the 'scene.xml' file in the '/unitree_robots/pr2/'or  '/unitree_robots/moying/' directory. 
# This file allows you to customize the loading of any of the robots.
robot_scene: "scene.xml" 

# DDS domain ID. It is recommended to use a different ID from the real hardware (real hardware defaults to 0).
domain_id: 1 

# Network interface name. For simulation, it is recommended to use local loopback "lo".
interface: "lo"

# Whether to print information about robot links, joints, sensors, etc. (Set 1 to enable).
print_scene_information: 1

# Whether to enable the virtual elastic band (Set 1 to enable).
# Mainly used to simulate the suspension process during the initialization of the H1 robot.
